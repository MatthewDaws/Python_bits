...
public:
	...
	static std::string Order_to_string(Order o);
...
Load_npy_Header::Order Load_npy_Header::get_order()const
{
...
std::string Load_npy_Header::Order_to_string(Order o)
{
	switch ( o ) {
		case Order::C: return "C";
		case Order::Fortran: return "Fortran";
	}
	return "Other";
}
...
private:
	std::string header;
	std::string dtype;
	Load_npy_Header::Order ordering;
...
Load_npy::Load_npy(std::istream &input)
{
	...
	try {
		...
		if ( ordering != Load_npy_Header::Order::C ) {
...
{
	std::ifstream file;
	file.open("int32.npy", std::ios::binary);
	try {
		Load_npy npy(file);
		cout << npy.info() << endl;
		if ( npy.check_format("f8") and npy.dimensions()==2 and npy.get_shape()[1]==2 ) {
